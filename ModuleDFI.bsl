...
//  ПрефиксДействия - Строка - Автоматически добавляемый префикс процедуры действия команды
Процедура СоздатьКоманду(Форма, ИмяКоманды, Заголовок, ИмяДействия, ПрефиксДействия = "") Экспорт
	
	НоваяКоманда = Форма.Команды.Добавить(ИмяКоманды);
	НоваяКоманда.Заголовок = Заголовок;
	Если ПрефиксДействия = "" Тогда
		НоваяКоманда.Действие = ИмяДействия;
	Иначе
		НоваяКоманда.Действие = СтрШаблон(ПрефиксДействия + "_%1", ИмяДействия);
	КонецЕсли;
...
//	ЭлементПеред	- Строка - достаточно указать название элемента формы перед которым требуется вставить новый элемент
//					- Элементы - можно передать элемент формы
//	Гиперссылка - вывод в режиме гиперссылки
//
...
//	                   Для сложных случаев лучше передать "неопределено" и воспользоваться процедурой формирования колонок таблицы формы этого же модуля ДФИ
...
//	ТочностьЧисла - Число - количество символов после запятой
//
Функция кЧисло(ДлинаЧисла = 15,ТочностьЧисла = 2)
...
//  ДатаВремя - Строка - Варианты: "ДатаВремя", "Дата" или "Время"
//
Функция кДатаВремя(ДатаВремя = "ДатаВремя")
...
Процедура УстановитьПараметрыЗапросу(РеквизитДинамическийСписок, ПараметрыЗапроса) 
